...
if(isset($_POST['submit'])){
   ...
   // Consulta para verificar se o email e senha estão corretos, e o status do usuário
   $select = mysqli_query($conn, "SELECT * FROM `user_form` WHERE email = '$email' AND password = '$pass'") or die('query failed');

   if(mysqli_num_rows($select) > 0){
      $row = mysqli_fetch_assoc($select);
      
      // Verificar se o status do usuário é 'banido'
      if($row['status'] == 'banido'){
         $message[] = 'Sua conta foi banida. Você não pode acessar o sistema.';
      } else {
         // Verifica se o usuário foi verificado
         if($row['verified'] == 1) {
            // Se o usuário foi verificado, armazena as informações do usuário na sessão
            $_SESSION['user_id'] = $row['id'];
            $_SESSION['user_name'] = $row['name'];
            $_SESSION['user_email'] = $row['email'];
            header('location:user.php');
         } else {
            // Se o usuário não foi verificado, exibe uma mensagem e redireciona para a página de verificação
            $message[] = 'Por favor, verifique seu e-mail antes de continuar.';
    ...
    <div class="body d-md-flex align-items-center justify-content-between" style="box-shadow: 0 0.5rem 1rem rgb(183 183 183);">
        ...
        <div class="box-2 d-flex flex-column h-100">
            <div class="mt-5">
                ...
                <p class="text-muted mb-2" style="font-size: 20px;"> E ensine novas gerações.</p>
                <div class="d-flex flex-column">
                    <br>
                    <p class="text-muted mb-2" style="font-size: 15px;">Conecte-se com...</p>
                    <div class="form-container">
                        <form action="" method="post" enctype="multipart/form-data">
                            ...
                            <div class="form-floating mb-3">
                                <input type="email" name="email" placeholder="enter email" class="form-control" style="width: 100%;" required>
                                <label for="email">Email</label>
                            </div>
                            <div class="form-floating mb-3">
                                <input type="password" name="password" placeholder="enter password" class="form-control" style="width: 100%;" required>
                                <label for="password">Senha</label>
                            </div>
                            <div class="d-grid">
                                <input type="submit" name="submit" value="Logar" style="width: 100%;" class="btn btn-dark">
                            </div>
                            <p class="footer text mb-0 mt-md-4 mt-4">Não tem uma conta? 
                                <span class="p-color me-1"><a href="register.php" class="p-color me-1">Registre-se</a></span>
                            </p>
                        </form>
                    </div>
                </div>
            ...
            <div class="mt-auto">
                <p class="footer text-muted mb-0 mt-md-0 mt-4">Para registrar você deve concordar com os
